...
# Finding the total emissions for each year in Baltimore City, MD
NEI1 <- select(NEI, Emissions, year, fips)[NEI$fips == 24510,]
NEI2 <- summarize(group_by(NEI1, year), total = sum(Emissions, na.rm = TRUE))

# Creating plot2 as a barplot
png("plot2.png")
...
barplot(NEI2$total,
        ...
        main = "Yearly Total PM2.5 Emissions in Baltimore City, MD")
