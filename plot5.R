...
# Finding the total emissions for each year for motor vehicles
mv <- SCC[grepl("Vehicle", SCC$EI.Sector),]
mvSCC <- mv$SCC
NEI1 <- NEI[NEI$fips == "24510",]
NEI2 <- NEI1[NEI1$SCC %in% mvSCC,]
NEI3 <- summarize(group_by(NEI2,  year), total = sum(Emissions, na.rm = TRUE))
...
p <- ggplot(NEI3, aes(x = as.factor(year), y = total) ) +
  ...
  ggtitle("Yearly Total Motor Vehicle PM2.5 Emissions in Baltimore City, MD")
